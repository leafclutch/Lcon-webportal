...
export function AnnouncementsClient({ announcements, canCreate, currentUserId }: {
  announcements: Announcement[]; canCreate: boolean; currentUserId: string
}) {
  ...
  const handleCreate = () => {
    if (!form.title || !form.content) { setError('All fields required'); return }
    startTransition(async () => {
      const res = await createAnnouncement(form)
      if (res.success) { setShowModal(false); setForm({ title: '', content: '' }) }
  ...
  return (
    <div className="space-y-4">
      ...
      <Modal open={showModal} onClose={() => setShowModal(false)} title="New Announcement" className="max-w-lg">
        <div className="space-y-4">
          ...
          <Textarea label="Content" value={form.content} onChange={e => setForm(f => ({ ...f, content: e.target.value }))} placeholder="What do you want to announce?" className="min-h-[120px]" />
          <div className="flex gap-3">
            <Button variant="outline" onClick={() => setShowModal(false)} className="flex-1">Cancel</Button>
            <Button onClick={handleCreate} loading={isPending} className="flex-1">Post Announcement</Button>
          </div>
        </div>
      </Modal>
    </div>
